...
export const CommanderScreen = () => {
  ...
  const groups: GroupDescriptor[] = useMemo(() => {
    ...
    return GROUP_ROLES.map((role) => {
      ...
      const flaggedId =
        groupState.flaggedOutOfTurn && current - groupState.flaggedOutOfTurn.flaggedAt <= 15_000
  ...
  return (
    <div className="flex min-h-full flex-1 flex-col gap-4 text-slate-100">
      ...
      <section className="rounded-lg border border-slate-800 bg-slate-900/70 px-4 py-3 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <p className="text-xs uppercase tracking-[0.3em] text-slate-400">指挥控制区</p>
            <h2 className="text-2xl font-semibold">
              战斗计时 {timerInfo.isRunning ? '进行中' : '未开始'}
            </h2>
          </div>
          <div className="flex flex-wrap items-center gap-5 text-sm">
            <div className="flex items-baseline gap-2">
              <span className="text-xs text-slate-400">战斗时间</span>
              <span className="font-mono text-xl">{timerInfo.battleClock}</span>
            </div>
            <div
              className={cn(
                'flex items-baseline gap-2 rounded-md px-3 py-2',
                timerInfo.showAlert ? 'bg-amber-500/20 text-amber-200' : 'bg-slate-800/60 text-slate-200'
              )}
            >
      ...
      <section className="grid min-h-0 flex-1 grid-cols-1 gap-4 lg:grid-cols-3">
        {groups.map((group) => (
          <div
            key={group.role}
            className="flex min-h-0 flex-col rounded-lg border border-slate-800 bg-slate-950/60 p-3 shadow-sm"
          >
            <header className="mb-3 flex items-center justify-between">
              <div>
                <p className="text-xs uppercase tracking-widest text-slate-500">{group.label}</p>
                <p className="text-sm text-slate-300">
                  下一位释放：{group.nextCaster ? group.nextCaster.name : '待加入'}
                </p>
              </div>
              <span className="text-[11px] uppercase tracking-[0.3em] text-slate-500">{group.members.length} 人</span>
            </header>
            <div
              ...
              {group.members.map((member) => {
                ...
                return (
                  <div
                    ...
                    onDragOver={(event) => event.preventDefault()}
                    className={cn(
                      'rounded-md border border-slate-700/80 bg-slate-950/70 px-4 py-3 text-sm transition',
                  ...
                  >
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-2">
                        ...
                        {isNext && (
                          <span className="rounded-md bg-emerald-500/20 px-1.5 py-0.5 text-[10px] text-emerald-200">NEXT</span>
                        )}
                      </div>
                    ...
                    <div className="mt-2 flex items-center justify-between text-[11px] text-slate-400">
                      ...
                      {isFlagged && <span className="text-amber-300">提前释放，请注意重排</span>}
                    </div>
                  </div>
                )
              })}
              {group.members.length === 0 && (
                <div className="flex h-full min-h-[120px] items-center justify-center rounded-md border border-dashed border-slate-700/70 text-sm text-slate-500">
                  等待该职业成员加入
                </div>
              )}
            </div>
